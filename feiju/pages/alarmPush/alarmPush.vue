<template>
	<view style="width: 100vw; height: 100vh; background-color: #FAFAFB;">
		...
		<view class="contenter">
			...
			<scroll-view scroll-y="true" style="height: calc(100vh - 250rpx);" v-if="alarmList.length > 0" @scrolltolower="bottomRefresh">
...
<script setup>
	...
	import { apiGetRealTimeAlarm} from '@/api/apis.js';
	...
	import { PROJECTID } from '@/config.js';
		...
		const getAlarm = async ()=>{
			...
			res = await apiGetRealTimeAlarm(params);
			// 调用提取数组处理方法
			const allArrayDsta = flattenTopLevelArrays(res.data);
			alarmList.value = [...alarmList.value , ...allArrayDsta];//数据拼接
